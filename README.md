...
```bash
curl -sL https://raw.githubusercontent.com/devohmycode/OhMyTermux/1.1.02/install.sh -o install.sh && chmod +x install.sh && ./install.sh --gum
```
...
```bash
curl -sL https://raw.githubusercontent.com/devohmycode/OhMyTermux/1.1.02/install.sh -o install.sh && chmod +x install.sh && ./install.sh
```
...
--lang | -l             # Language selection (en, fr)
...
Version 1.1.0 :
 - Added `--lang | -l` argument for language selection (en, fr)
 - Updated Gum interface to include language selection
 - Updated README files (en, fr)
 - Improved error handling and user prompts in installation script
